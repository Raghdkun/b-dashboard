...
interface MaintenanceState {
  ...
  // Actions
  fetchRequests: (storeId?: string, limit?: number) => Promise<void>;
...
export const useMaintenanceStore = create<MaintenanceState>()((set, get) => ({
  ...
  fetchRequests: async (storeId?: string, limit: number = 10) => {
    const state = get();
  ...
  refreshRequests: async () => {
    const { lastStoreId, lastLimit, fetchRequests } = get();
    if (lastStoreId) {
      await fetchRequests(lastStoreId, lastLimit);
    }
  },
