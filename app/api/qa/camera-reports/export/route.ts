...
const UPSTREAM_TIMEOUT_MS = Number(process.env.QA_TIMEOUT_MS) || 30_000;
const MAX_RETRIES = 1;
...
/* ────────────────────────────────────────────────────────────────────────── */
/*  GET /api/qa/camera-reports/export                                       */
/* ────────────────────────────────────────────────────────────────────────── */
...
export async function GET(request: NextRequest) {
  ...
  const targetUrl = `${QA_BASE_URL}/camera-reports/export${queryString ? `?${queryString}` : ""}`;
  ...
  try {
    ...
    if (response.ok) {
      ...
      const contentDisposition =
        ...
        'attachment; filename="camera-report-export.xlsx"';
