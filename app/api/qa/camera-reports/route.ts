...
export async function GET(request: NextRequest) {
  ...
  const targetUrl = `${QA_BASE_URL}/camera-reports${queryString ? `?${queryString}` : ""}`;
  ...
  try {
    ...
    if (response.status === 403) {
      return errorResponse(
        "FORBIDDEN",
        "You do not have permission to access camera reports.",
        403
      );
    }
    if (response.status === 404) {
      return errorResponse(
        "NOT_FOUND",
        "Camera reports endpoint not found.",
        404
      );
    }
